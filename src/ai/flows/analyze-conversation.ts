...
9.  **Genera el Resultado JSON (estructura \`AnalyzeConversationOutputSchema\`):**
    ...
    *   **recomendaciones**: Proporciona recomendaciones específicas, detalladas y accionables. **Adapta según \`posible_agresor\`, \`persona_afectada\` y \`riesgo_inminente\`**:
        ...
        *   **Si \`riesgo_inminente\` es \`true\`**:
            ...
            *   (Si el afectado es el interlocutor y el usuario no es el agresor): "Si crees que [interlocutor] corre peligro inmediato y es seguro para ti, contacta servicios de emergencia o informa a alguien cercano a esa persona."
        ...
        *   **Si \`riesgo_inminente\` es \`false\` (ajusta según \`posible_agresor\` y \`persona_afectada\`):**
            ...
            *   **Recomendaciones Específicas y Accionables (Adaptadas):**
                ...
                *   **Si \`persona_afectada\` es \`interlocutor\` (y \`posible_agresor\` es \`usuario\` o \`ambiguo\`):**
                    *   **Autoconciencia Profunda:** "El análisis sugiere que tu comportamiento, ejemplificado por frases como '[ejemplo]', se alinea con patrones de [categoría detectada]. Es fundamental que reconozcas el impacto negativo que esto tiene en [interlocutor], pudiendo causar [impacto específico, ej. 'miedo, ansiedad, tristeza, resentimiento, sensación de caminar sobre cáscaras de huevo']."
                    ...
                    *   **Aprendizaje de Alternativas Saludables:**
                        ...
                        *   "**Desarrolla la Empatía:** Intenta ponerte activamente en el lugar de [interlocutor]. ¿Cómo perciben ellos tus palabras/acciones? Escucha activamente su perspectiva sin interrumpir ni invalidar."
                    ...
                    *   **Responsabilidad y Reparación:** "Asumir la responsabilidad por el daño causado es esencial. Una disculpa sincera (sin excusas) y un cambio de comportamiento demostrable son necesarios si deseas reparar la relación. Considera preguntar a [interlocutor] qué necesita para sentirse seguro/a de nuevo."
